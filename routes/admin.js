// ============================================================
// routes/admin.js — Super Admin: User Management & System Settings
// ============================================================
...
router.post('/users/toggle/:id', validateParamId(), async (req, res) => {
    try {
        ...
        const [[user]] = await db.query('SELECT is_active FROM system_user WHERE user_id = ?', [req.params.id]);
...
// POST /admin/users/unlock/:id — unlock a locked account
...
// POST /admin/users/reset-password/:id — force reset password
...
// GET /admin/audit — system audit log (shows recent activity)
router.get('/audit', async (req, res) => {
    try {
        ...
        res.render('admin/audit', {
            title: 'System Audit',
            recentDonations, recentRequests, recentLogins
        });
    } catch (err) {
        console.error(err);
        res.status(500).send('Server error');
